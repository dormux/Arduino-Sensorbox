...
float Adafruit_MPL3115A2::getPressure() {
  ...
  _i2c->requestFrom((uint8_t) MPL3115A2_ADDRESS, (uint8_t) 3, (uint32_t) MPL3115A2_REGISTER_PRESSURE_MSB, (uint8_t) 3, true);
  #else
...
float Adafruit_MPL3115A2::getAltitude() {
  ...
  #if defined(__SAM3X8E__)
  _i2c->requestFrom((uint8_t) MPL3115A2_ADDRESS, (uint8_t) 3, (uint32_t) MPL3115A2_REGISTER_TEMP_MSB, (uint8_t) 3, true);
  #else
  _i2c->beginTransmission(MPL3115A2_ADDRESS); // start transmission to device 
  _i2c->write(MPL3115A2_REGISTER_PRESSURE_MSB); 
  _i2c->endTransmission(false); // end transmission
...
float Adafruit_MPL3115A2::getTemperature() {
  ...
  #if defined(__SAM3X8E__)
  _i2c->requestFrom((uint8_t) MPL3115A2_ADDRESS, (uint8_t)2, (uint32_t) MPL3115A2_REGISTER_TEMP_MSB, (uint8_t) 1, true);
  #else
  _i2c->beginTransmission(MPL3115A2_ADDRESS); // start transmission to device 
...
uint8_t Adafruit_MPL3115A2::read8(uint8_t a) {
  #if defined(__SAM3X8E__)
  _i2c->requestFrom((uint8_t) MPL3115A2_ADDRESS, (uint8_t) 1, (uint32_t) a, (uint8_t) 1, true);
  #else
  _i2c->beginTransmission(MPL3115A2_ADDRESS); // start transmission to device 
